...
contract DynamicOracle is IDynamicOracle {
    ...
    constructor(address factory_, uint256 windowSize_, uint8 granularity_) {
        require(granularity_ > 1, "Granularity not greater than 0");
        require(
    ...
    function getFirstObservationInWindow(address pair) public view returns (Observation memory firstObservation) {
        ...
        uint8 firstObservationIndex = (observationIndex + 1) % uint8(granularity);
    ...
    function computeAmountOut(
        ...
        amountOut = (uint256(priceAverage.decode()) * amountIn);
        }
    }
    ...
    function consult(address tokenIn, uint256 amountIn, address tokenOut) external view returns (uint256 amountOut) {
        ...
        (uint256 price0Cumulative, uint256 price1Cumulative,) = OracleLibrary.currentCumulativePrices(pair);
        (address token0,) = Library.sortTokens(tokenIn, tokenOut);
